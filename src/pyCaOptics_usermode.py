...
def load_tenant_config(config_path):
    ...
    The file should contain a list of objects with 'tenant_id' keys,
...
def main(tenant_id=None, config_path=None):
    tenants = []
    if config_path:
        ...
        print("Must provide either a tenant_id or a --config config file path.")
        sys.exit(1)

    for tenant in tenants:
        analyze_tenant(tenant["tenant_id"])

def analyze_tenant(tenant_id):
    try:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='Analyze Azure Conditional Access policies (user/device-code mode).')
    ...
    if args.config:
        ...
        main(tenant_id=args.tenant_id)
    else:
        print("Usage: python pyCaOptics_usermode.py <tenant_id>")
        print("       python pyCaOptics_usermode.py --config <config.json|config.yaml>")
        sys.exit(1)
